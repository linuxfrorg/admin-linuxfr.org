...
upstream linuxfr-frontend {
    server unix:/www/<%= fqdn %>/<%= env %>/shared/tmp/sockets/<%= env %>.sock fail_timeout=0;
}

upstream board-frontend {
    server unix:/www/<%= fqdn %>/board/board.sock;
}
...
server {
    ...
    root /www/<%= fqdn %>/<%= env %>/current/public;
...
<% end %>
    ...
    location ~* \.(php|asp|aspx|labels.rdf|apple-touch-icon(-precomposed)?.png)$ {
      return 404;
    }
